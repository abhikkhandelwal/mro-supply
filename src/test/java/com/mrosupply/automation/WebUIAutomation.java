...
public class WebUIAutomation extends TestBase {
	...
	public static boolean checkTestCaseRunmode(String testCase){
		...
		for(int i=2; i<=excel.getRowCount("MRO-Supply - Test Cases"); i++){
			
			if(excel.getCellData("MRO-Supply - Test Cases", "Test Case ID", i).equalsIgnoreCase(testCase)){
				
				if(excel.getCellData("MRO-Supply - Test Cases", "Runmode", i).equalsIgnoreCase("Y")){
